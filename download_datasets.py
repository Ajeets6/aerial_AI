...
def install_dependencies(logger):
    ...
    required_packages = ['kaggle', 'gdown', 'requests', 'python-dotenv']
...
def download_solar_panels(data_dir, logger):
    """Download solar-antwerp dataset from GitHub"""
    logger.info("\n" + "="*80)
    logger.info("Dataset 2/3: Solar Panels (solar-antwerp)")
    logger.info("="*80)
    
    repo_url = "https://github.com/patrick-tobben/solar-antwerp.git"
    output_dir = Path(data_dir) / "raw" / "solar_antwerp"
    
    success = download_github_repo(repo_url, output_dir, logger)
    ...
    if success:
        ...
        logger.info(f"Dataset location: {target_dir}")
        logger.info("Note: You may need to manually organize annotations into train/val folders")
    
    return success

def download_indiasat(data_dir, logger):
    """Download IndiaSat dataset"""
    logger.info("\n" + "="*80)
    logger.info("Dataset 3/3: IndiaSat")
    logger.info("="*80)
    
    dataset_name = "abhinav05/indiasat"
    output_dir = Path(data_dir) / "raw" / "indiasat"
    
    success = download_kaggle_dataset(dataset_name, output_dir, logger)
    
    if success:
        target_dir = Path(data_dir) / "indiasat"
...
def main():
    ...
    parser.add_argument(
        "--datasets",
        nargs='+',
        choices=['aerial', 'solar', 'india', 'all'],
        default=['all'],
    ...
    if not args.skip_kaggle_check and 'all' in args.datasets or 'aerial' in args.datasets or 'india' in args.datasets:
    ...
    if 'all' in datasets_to_download:
        datasets_to_download = ['aerial', 'solar', 'india']
    
    if 'aerial' in datasets_to_download:
        results['aerial'] = download_aerial_segmentation(args.data_dir, logger)
    
    if 'solar' in datasets_to_download:
        results['solar'] = download_solar_panels(args.data_dir, logger)
    
    if 'india' in datasets_to_download:
        results['indiasat'] = download_indiasat(args.data_dir, logger)
