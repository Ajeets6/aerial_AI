...
def validate(model, dataloader, device, logger, num_classes, use_amp):
    """Validate the model"""
    model.eval()
    total_loss = 0
    all_preds = []
...
def main():
    ...
    # System arguments
    parser.add_argument("--num_workers", type=int, default=0 if os.name == "nt" else min(4, os.cpu_count() or 1),
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_loss, val_miou, val_acc = validate(
            model, val_loader, device, logger, args.num_classes, args.use_amp and device.type == "cuda"
        )
